...
function gen_main_nav_list() {
    ...
    count($all_page_ids_original) > 1 ? asort($all_page_ids_original) : $all_page_ids_original;
    $reverse_all_page_ids = array_reverse($all_page_ids_original);
    $current_page_title = get_the_title();

    if (count($reverse_all_page_ids) > 0) {
        echo("<div class='main-nav__list'>");
            foreach ($reverse_all_page_ids as $page) {
                $page_status = get_post_status($page);
